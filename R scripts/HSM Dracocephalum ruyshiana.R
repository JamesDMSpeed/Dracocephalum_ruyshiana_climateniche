...
grid.arrange(pt,pp,pps,pph,pB,ncol=3)
#dev.off()
dev.off()
...
levelplot(drapred_mean,scales=list(draw=F),names.attr=c('C. lepidocarpa','C. jemtlandica'))
...
ens_dracep<-ensemble(sdm1,newdata = predvars,filename = 'Outputs/Model Predictions/dracep_ensemble',
                  setting=list(method='weighted',stat='AUC',overwrite=T,
                               id=sdm1@run.info$modelID),overwrite=TRUE)
ens_dracep<-raster('ModelPredictions/dracep_ensemble')
...
levelplot(ens_dracep,scales=list(draw=F),par.settings='YlOrRdTheme',
          cex=1.5)+
  layer(sp.polygons(norwayP,col=grey(0.5)))
dev.off()
dev.off()
...
niche_temp_pc_dracep<-niche(predvars,ens_dracep,c('bio10_16','bio15_16'),plot=F)

#Stack up niches

levelplot(nniche_temp_pc_dracep,par.settings='YlOrRdTheme')
...
niche_temp_map_dracep<-niche(predvars,ens_dracep,c('bio10_16','bio12_16'),plot=F)
